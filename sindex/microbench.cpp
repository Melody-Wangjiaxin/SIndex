...
std::vector<key_type> read_weblogs(const char *path) 
{
  ...
  // while (true) {
  //   size_t num_read = fread(buf, sizeof(key_type), 2048, fin);
  //   for (int i = 0; i < num_read; i++) {
  //     vec.push_back(buf[i]);
  //   }
  //   if (num_read < 2048) break;
  // }
  // fclose(fin);
  ...
  size_t data_size;
  is.read((char *) &data_size, sizeof(uint64_t));
...
void weblogs_data() {
  std::vector<key_type> read_keys = read_weblogs<key_type>("/home/wjx/sindex/data/weblogs_unix.uint64");
  size_t size = read_keys.size();
  COUT_THIS("read_keys.size(): " << read_keys.size());
...
template<typename key_type>
std::vector<key_type> read_maps(const std::string& path) 
{
  std::vector<key_type> vec;
  double tmp_d;
  key_type tmp_i;
  ...
  while(is.read((char *)&tmp_d, sizeof(tmp_d)))
  {
    memcpy(&tmp_i, &tmp_d, sizeof(tmp_d));
    vec.push_back(tmp_i);
  }
  is.close();
  return vec;
...
void map_data() {
  std::vector<key_type> read_keys = read_maps<key_type>("/home/wjx/sindex/data/longitudes-200M.bin.data");
  COUT_THIS("read_keys.size(): " << read_keys.size());
  ...
  //     k.buf[i] = (uint8_t)read_keys[i + j * sizeof(index_key_t)];
  //   }
...
template<typename key_type>
std::vector<key_type> read_longlat(const std::string& path) 
{
  std::vector<key_type> vec;
  double tmp_d;
  key_type tmp_i;
...
void longlat_data() {
  std::vector<key_type> read_keys = read_longlat<key_type>("/home/wjx/sindex/data/longlat-200M.bin.data");
  COUT_THIS("read_keys.size(): " << read_keys.size());
  size_t num = table_size;
  exist_keys.reserve(num);
  for (size_t j = 0; j < num; ++j) {
...
template<typename key_type>
std::vector<key_type> read_lognormal(const std::string& path) 
{
  std::vector<key_type> vec;
  int64_t tmp_i;
  std::ifstream is(path.c_str(), std::ios::binary | std::ios::in);
...
void lognormal_data() {
  std::vector<key_type> read_keys = read_lognormal<key_type>("/home/wjx/sindex/data/lognormal-190M.bin.data");
  COUT_THIS("read_keys.size(): " << read_keys.size());
  size_t num = table_size;
  exist_keys.reserve(num);
  for (size_t j = 0; j < num; ++j) {
...
template<typename key_type>
std::vector<key_type> read_ycsb(const std::string& path) 
{
  std::vector<key_type> vec;
  uint64_t tmp_i;
  std::ifstream is(path.c_str(), std::ios::binary | std::ios::in);
...
void ycsb_data() {
  std::vector<key_type> read_keys = read_ycsb<key_type>("/home/wjx/sindex/data/ycsb-200M.bin.data");
  COUT_THIS("read_keys.size(): " << read_keys.size());
  size_t num = table_size;
  exist_keys.reserve(num);
  for (size_t j = 0; j < num; ++j) {
...
void load_data() {
  if(data_type == "normal"){
    rand_data();
  }else if(data_type == "weblogs"){
    weblogs_data();
  }else if(data_type == "map"){
    map_data();
  }else if(data_type == "longlat"){
    longlat_data();
  }else if(data_type == "lognormal"){
    lognormal_data();
  }else if(data_type == "ycsb"){
    ycsb_data();
  }else{
    rand_data();
  }
...
inline void parse_args(int argc, char **argv) {
  struct option long_options[] = {
      ...
      {"data-type", required_argument, 0, 's'},
      {0, 0, 0, 0}};
  std::string ops = "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:r:s:";
  int option_index = 0;
