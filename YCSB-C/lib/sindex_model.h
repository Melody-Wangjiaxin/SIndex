...
inline void model_prepare(const std::vector<double *> &model_key_ptrs,
  ...
  if (feature_len == 1) {
    ...
    // 每个group有feature_len个double，每个double是一个feature，收集其中有用的feature
    std::vector<size_t> useful_feat_index;
    for (size_t feat_i = 0; feat_i < feature_len; feat_i++) {
      double first_val = model_key_ptrs[0][feat_i]; // 第一个key指针指向double数组中的第feat_i个
