...
void rcu_init(size_t start_num, size_t worker_num) {
  sindex::config_mutex.lock();
  if (sindex::config.rcu_status.get() == nullptr) {
    sindex::config.rcu_status = std::make_unique<sindex::rcu_status_t[]>(start_num + worker_num);
    for (size_t worker_i = start_num; worker_i < worker_num; worker_i++) {
      sindex::config.rcu_status[worker_i].status = 0;
      sindex::config.rcu_status[worker_i].waiting = false;
    }
  }
  sindex::config_mutex.unlock();
}
